...
def ren_handover_seq(handover_seq):
    ...
    for i, node in enumerate(handover_seq):
        if node <> '0':
            renamed_seq.append(node + '_' + str( handover_seq[:i+1].count(node) ) )
    # print renamed_seq
    return renamed_seq
...
def read_handovers(csv_reader):
	...
	for line in csv_reader:
		lines.append( [node for node in line] )
	return lines
...
def write_matchs(match_tab, csv_writer):
...
def main(argv=None):
	...
	write_matchs(match_tab, match_writer)
